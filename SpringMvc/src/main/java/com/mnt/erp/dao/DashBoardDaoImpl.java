...
String hql=null;
	...
	public List<Object[]> getTotalStocks(int storageLocId) {
		...
		try
		{
			hql="select m.matStockId,m.batchNo,m.qtyAval,m.storageLocDetails,m.materialsDetails from com.mnt.erp.bean.MatStockBean m where m.storLocId="+storageLocId;
	...
	public List<PendingPo> getPendingPoDetails(String count) {
		...
		try
		{
			...
			if(count.equals("all"))
			{
				...
				/*PendingPo Procedure configration is ther in PurchaseOrder.hbm.xml file*/
				listofObjects=getHibernateTemplate().findByNamedQuery("PendingPo");
			}
			else
			{
				/*TOP5PendingPo Procedure configration is ther in PurchaseOrder.hbm.xml file*/
				listofObjects=getHibernateTemplate().findByNamedQuery("TOP5PendingPo");
			}
	...
	public List<GoodsDeliveryInTheYear> getGoodsinyear(String msg) {
		...
		if(msg.equals("all"))
		{
			listofObjects=getHibernateTemplate().findByNamedQueryAndNamedParam("GoodsDeliveryIntheYear","message","all");
	
		}
		else
		{
			listofObjects=getHibernateTemplate().findByNamedQueryAndNamedParam("GoodsDeliveryIntheYear","message","top5");
	...
	public List<ToBeReoredeMaterial> getTobeReorederLevels(String msg) {
		...
		try
		{
			if(msg.equals("top5"))
			{
			listofObjects=getHibernateTemplate().findByNamedQueryAndNamedParam("TobeReorderedMaterial","exMessage","top5");
			}
			else
			{
				listofObjects=getHibernateTemplate().findByNamedQueryAndNamedParam("TobeReorderedMaterial","exMessage","all");	
			}
	...
	public List<InvoiceToCollect> invoiceToCollect(String count) {
		...
		try
		{
			...
			{
				/*TOP5GetInvoiesToCollect Procedure configration is ther in CustomerInvoice.hbm.xml file*/
				listofObjects=getHibernateTemplate().findByNamedQuery("TOP5GetInvoiesToCollect");
			}
			else
			{
				/*GetInvoiesToCollect Procedure configration is ther in CustomerInvoice.hbm.xml file*/
				listofObjects=getHibernateTemplate().findByNamedQuery("GetInvoiesToCollect");
			}
	...
	public List<InvoiceToPay> getInvoiceToPays(String count) {
		...
		try
		{
			if(count.equals("all"))
			{
				/*InvoiceToPay Procedure configration is ther in VendorInvoice.hbm.xml file*/
				listofObjects=getHibernateTemplate().findByNamedQuery("InvoiceToPay");
			}
			else
			{
				/*TOP5InvoiceToPay Procedure configration is ther in VendorInvoice.hbm.xml file*/
				listofObjects=getHibernateTemplate().findByNamedQuery("TOP5InvoiceToPay");
			}
	...
	public List<Object[]> getTop5orderItems() {
		...
		try
		{
			listofObjects=getHibernateTemplate().findByNamedQuery("top5OrderItems");
...
catch(Exception e)
{
	...
	public List<Object[]> getSalesOrdersByYear(int year) {
		...
		try
		{
			listofObjects=getHibernateTemplate().findByNamedQueryAndNamedParam("GetSOrderPCTbyYear","Year",year);
	...
	public List<Object[]> allorderItems() {
		try
		{
			listofObjects=getHibernateTemplate().findByNamedQuery("allOrderItems");
			 logger.debug("top5orderitems size iss=="+listofObjects.size());
		}
catch(Exception e)
{
