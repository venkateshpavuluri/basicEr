...
public class MaterialDaoImpl extends HibernateDaoSupport implements MaterialDao {
	...
	public List<Object[]> searchMaterialWithId(int id) {
		...
		try {
			...
			String hql = "select m.material_Id,m.materialCode,m.materialName,m.materialTypeValues,m.uomDetails,m.taxCatogery,m.category,m.materialDescription,m.salesUOM,m.active,m.purchaseUOM,m.avgWeight,m.avgHeight,m.avgLength,m.avgVolume,m.weightUOM,m.heightUOM,m.lengthUOM,m.dimension,m.shelfLife,m.shelfLifeUOM,m.reorderLevel,m.maxDeliveryTime,m.maxDeliveryTimeUOM,m.alternateUOM from Material m where m.material_Id="
					+ id + "";
