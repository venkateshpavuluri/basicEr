	...
	public List<Object> getVendorReturnAdvance(String columns, String opeator,
		...
		for (int i = 0; i < advanceSearch.length; i++) {
			if (!op[i].equals("0") && advanceSearch[i] != "") {
				...
				if (!op[i].equals("0") && advanceSearch[i] != "") {
					finalStringForSearch = finalStringForSearch + "  vr."
		...
		if (finalStringForSearch.length() > 3) {
			advSearchList = vendorReturnDao
					.vendorReturnAdvanceSearch(finalStringForSearch.substring(
							0, finalStringForSearch.length() - 3));
		} else {
			advSearchList = vendorReturnDao.vendorReturnAdvanceSearch("ALL");
		}
