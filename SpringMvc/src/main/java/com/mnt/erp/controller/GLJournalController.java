...
public class GLJournalController {
	...
	public ModelAndView glJournalHome(
		...
		List<String> list = menuService.getPrivilige(
				"glJournalHome.mnt", session.getAttribute("userId")
						.toString());
	...
	public String saveglJournal(
		...
		journalBean.setCreatedDTTM(df.format(date));
		...
		try {
			glJournalBean.setGlAccountDT(dateService.dateFormat(dateService.dateParse(glJournalBean.getGlAccountDT(),"au"),"au"));
			glJournalBean.setPostingDT(dateService.dateFormat(dateService.dateParse(glJournalBean.getPostingDT(),"au"),"au"));
			...
			if (flag == true) {
				...
				auditLogService.setAuditLogSave(session.getAttribute("userId")
						.toString(),"A","GL Journal","ROW" ,String.valueOf(journalBean.getGlAccountId()),"1",modifiedDate,session.getAttribute("userName").toString());
	...
	public String searchglJournal(
		...
		try {
			...
			while (itr.hasNext()) {
				...
				dnb.setGlAccountDT(dateService.dateFormat(dateService.dateParse((String) objects[1],"se"),"se"));
				dnb.setPostingDT(dateService.dateFormat(dateService.dateParse((String) objects[2],"se"),"se"));
	...
	public String glJournalDelete(
		...
		try {
			...
			if (flag == true) {
				...
				String modifiedDate= new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date);
				auditLogService.setAuditLogSave(session.getAttribute("userId")
						.toString(),"D","GL Journal","ROW" ,String.valueOf(glId),"1",modifiedDate,session.getAttribute("userName").toString());
	...
	public String glJournalEdit(
		...
		try {
			...
			if (itrr.hasNext()) {
				...
				glJournalBeanEdit.setEglAccountDT(dateService.dateFormat(dateService.dateParse(sob.getGlAccountDT(),"se"),"se"));
				glJournalBeanEdit.setEpostingDT(dateService.dateFormat(dateService.dateParse(sob.getPostingDT(),"se"),"se"));
	...
	public String glJournalUpdate(
		...
		Date date = new Date();
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
		glJournalBeanUpdate.setModifiedDTTM(df.format(date));
		...
		try {
			glJournalBeanUpdate.setGlAccountDT(dateService.dateFormat(dateService.dateParse(glJournalBeanUpdate.getEglAccountDT(),"au"),"au"));
			glJournalBeanUpdate.setPostingDT(dateService.dateFormat(dateService.dateParse(glJournalBeanUpdate.getEpostingDT(),"au"),"au"));
