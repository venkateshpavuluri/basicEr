		...
		if (purReqDuplicate == 0) {
			try {
				...
				if (materialedit != null) {
					...
					purchaseReq.getRedates();
					purchaseReq.setRedates(dateService.dateFormat(dateService
							.dateParse(purchaseReq.getRedates(), "au"), "au"));
					...
					for (int i = 0; i < quantity.length; i++) {
						...
						purchaseReqLine.setRequiredDate(rdates[i].toString());
