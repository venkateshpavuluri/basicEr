...
public class DebitNoteController {
	...
	public ModelAndView debitNoteHome(
		...
		List<String> list = menuService.getPrivilige(
				"debitNoteHome.mnt", session.getAttribute("userId")
						.toString());
	...
	public String saveDebitNote(
		...
		try {
			debitNoteBean.setDebitNoteDT(dateService.dateFormat(dateService.dateParse(debitNoteBean.getDebitNoteDT(),"au"),"au"));
			...
			if (flag == true) {
				...
				auditLogService.setAuditLogSave(session.getAttribute("userId")
						.toString(),"A","Debit Note","ROW" ,String.valueOf(debitBean.getDebitNoteId()),"1",modifiedDate,session.getAttribute("userName").toString());
	...
	public String searchDebitNote(
		...
		try {
			...
			while (itr.hasNext()) {
				...
				dnb.setDebitNoteDT(dateService.dateFormat(dateService.dateParse((String) objects[2],"se"),"se"));
	...
	public String debitNoteDelete(
		...
		try {
			...
			if (flag == true) {
				...
				String modifiedDate= new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(date);
				auditLogService.setAuditLogSave(session.getAttribute("userId")
						.toString(),"D","Debit Note","ROW" ,String.valueOf(dId),"1",modifiedDate,session.getAttribute("userName").toString());
	...
	public String debitNoteEdit(
		...
		try {
			...
			if (itrr.hasNext()) {
				...
				debitNoteBeanEdit.setEdebitNoteDT(dateService.dateFormat(dateService.dateParse(sob.getDebitNoteDT(),"se"),"se"));
	...
	public String debitNoteUpdate(
		...
		debitNoteBeanUpdate.setCustInvoiceId(debitNoteBeanUpdate.getEcustInvoiceId());
		debitNoteBeanUpdate.setVendorInvoiceId(debitNoteBeanUpdate.getEvendorInvoiceId());
		...
		try {
			debitNoteBeanUpdate.setDebitNoteDT(dateService.dateFormat(dateService.dateParse(debitNoteBeanUpdate.getEdebitNoteDT(),"au"),"au"));
