...
public class SalesInquiryController {
	...
	public ModelAndView salesInquiryHome(
		...
		List<String> list = menuService.getPrivilige(
				"salesInquiryHome.mnt", session.getAttribute("userId")
						.toString());
	...
	public String saveSalesInquiry(
		...
		if (materialId != null) {
			...
			for (int n = 0; n < materialId.length; n++) {
				...
				siLine.setRequiredDate(dateService.dateFormat(dateService.dateParse(reqDate[n], "au"), "au"));
			...
			siBean.setRequestedDate(dateService.dateFormat(dateService.dateParse(siBean.getRequestedDate(), "au"), "au"));
			siBean.setRequiredDeliveryDate(dateService.dateFormat(dateService.dateParse(siBean.getRequiredDeliveryDate(), "au"), "au"));
			siBean.setValidFrom(dateService.dateFormat(dateService.dateParse(siBean.getValidFrom(), "au"), "au"));
			siBean.setValidTo(dateService.dateFormat(dateService.dateParse(siBean.getValidTo(), "au"), "au"));
			...
			String msg=salesInquiryService.saveSalesInquiry(siBean);
				...
				auditLogService.setAuditLogSave(session.getAttribute("userId")
						.toString(),"A","Sales Inquiry","ROW" ,String.valueOf(siBean.getSalesInquiryId()),"1",modifiedDate,session.getAttribute("userName").toString());
				salesInquirySave = "Sales Inquiry Data Saved Successfully";
				}else{
					return "redirect:salesInquiryHome.mnt?addSEFail=" + salesInquirySave
							+ "";
				}
	...
	public String searchSalesInquiry(
		...
		try {
			...
			while (itr.hasNext()) {
				...
				sib.setRequestedDate(dateService.dateFormat(dateService.dateParse((String) objects[2], "se"), "se"));
				sib.setValidFrom(dateService.dateFormat(dateService.dateParse((String) objects[3], "se"), "se"));
				sib.setValidTo(dateService.dateFormat(dateService.dateParse((String) objects[4], "se"), "se"));
				sib.setRequiredDeliveryDate(dateService.dateFormat(dateService.dateParse((String) objects[5], "se"), "se"));
	...
	public String salesInquiryDelete(
		...
		try {
			...
			if (msg.equals("Deleted")) {
				...
				String modifiedDate= new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format( new Date());
				auditLogService.setAuditLogSave(session.getAttribute("userId")
						.toString(),"D","Sales Inquiry","ROW" ,String.valueOf(cId),"1",modifiedDate,session.getAttribute("userName").toString());
	...
	public String salesInquiryEdit(
		...
		try {
			...
			if (itrr.hasNext()) {
				...
				salesInquiryBeanEdit.setErequestedDate(dateService.dateFormat(dateService.dateParse(ccb.getRequestedDate(), "se"), "se"));
				salesInquiryBeanEdit.setErequiredDeliveryDate(dateService.dateFormat(dateService.dateParse(ccb.getRequiredDeliveryDate(), "se"), "se")
						);
				salesInquiryBeanEdit.setEvalidFrom(dateService.dateFormat(dateService.dateParse(ccb.getValidFrom(), "se"), "se"));
				salesInquiryBeanEdit.setEvalidTo(dateService.dateFormat(dateService.dateParse(ccb.getValidTo(), "se"), "se"));
				...
				while (iter.hasNext()) {
					...
					sMultiple.setErequiredDate(dateService.dateFormat(dateService.dateParse(cc.getRequiredDate(), "se"), "se"));
					Uom uom = cc.getUom();
	...
	public String salesInquiryUpdate(
		...
		SalesInquiryBeanUpdate.setDescription(SalesInquiryBeanUpdate
				.getEdescription());
		SalesInquiryBeanUpdate.setValidFrom(dateService.dateFormat(dateService.dateParse(SalesInquiryBeanUpdate.getEvalidFrom(), "au"), "au")
				);
		SalesInquiryBeanUpdate.setValidTo(dateService.dateFormat(dateService.dateParse(SalesInquiryBeanUpdate.getEvalidTo(), "au"), "au"));
		SalesInquiryBeanUpdate.setRequestedDate(dateService.dateFormat(dateService.dateParse(SalesInquiryBeanUpdate.getErequestedDate(), "au"), "au"));
				
		SalesInquiryBeanUpdate.setRequiredDeliveryDate(dateService.dateFormat(dateService.dateParse(SalesInquiryBeanUpdate.getErequiredDeliveryDate(), "au"), "au")
				);
		...
		if (matid != null) {
			...
			for (int n = 0; n < matid.length; n++) {
				...
				if (cbId == 0) {
					...
					sib.setRequiredDate(dateService.dateFormat(dateService.dateParse(reqDate[n], "au"), "au"));
			...
			if (msg == "Updated") {
				...
				return "redirect:salesInquiryHome.mnt?UpdateSEFail=" + salesUpdate + "";
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
