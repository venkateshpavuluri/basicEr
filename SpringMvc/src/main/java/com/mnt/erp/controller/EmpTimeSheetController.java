...
public class EmpTimeSheetController {
	...
	public String getBrTime(@ModelAttribute EmployeeTimeSheet btBean,
	...
	public String saveBrTime(@ModelAttribute("EmpTimeSheet") EmployeeTimeSheet empBean,
		...
		try {
			...
			if (id == 0) {
				...
				breaksuccessdup = "Warning ! EmpTimeSheet is already exists. Please try some other name";
				empBean.setEmphide(1);
				request.setAttribute("empsuccessdup", breaksuccessdup);
				return "EmpTimeSheet";
			}
	...
	public String searchBrTimeIds(@ModelAttribute("EmpTimeSheet") EmployeeTimeSheet eBean,
	...
	public String editBrTime(@ModelAttribute("EmpTimeSheet") EmployeeTimeSheet brBean,
	...
	public String updateBrTime(@ModelAttribute("EmpTimeSheet") EmployeeTimeSheet brBean,
			HttpServletRequest request, HttpServletResponse response,
			Model model) {
		response.setCharacterEncoding("UTF-8");
			...
			if (id == 0) {
			  ...
			  etsService.updateEmpTimeSheet(brBean);
			   ...
			   request.setAttribute("empTimeSheetUpdate", "emptimeSheet Details Updated Successfully");
				 ...
				 BreakTimeUpdate = "EmpTime Sheet already exists choose another one";
				
					return "redirect:EmpTimeSheet.mnt?BreakTimeUpdate="
					+ BreakTimeUpdate + "";
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	@RequestScoped
	public String brTimeDetailsDelete(@ModelAttribute("EmpTimeSheet") EmployeeTimeSheet brBean,HttpServletRequest request,
			HttpServletResponse response,Model model) {
